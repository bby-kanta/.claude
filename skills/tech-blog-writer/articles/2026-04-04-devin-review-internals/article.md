...
grepやfindでdiff外のファイルを読みに行くのは他のAIレビューツールでもやっていますが、他のツールとは一線を画す指摘の質の高さなので、何か独自の仕組みを入れているのではないかと想像しています。リポジトリインデックスとDeepWikiという検索基盤がすでにあって、チャット欄ではそれをAsk Devin経由で活用している。であれば、Bug Catcherも同じ基盤を使ってコードベースを探索していてもおかしくないなと。
