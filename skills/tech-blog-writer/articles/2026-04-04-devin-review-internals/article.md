...
[公式ドキュメント](https://docs.devin.ai/onboard-devin/knowledge-onboarding)によると:
...
[同じく公式ドキュメント](https://docs.devin.ai/product-guides/knowledge)には:
...
Devin はコードベースをバックグラウンドでインデックス化します。[Ask Devin のドキュメント](https://docs.devin.ai/work-with-devin/ask-devin)によると:
...
アーキテクチャ図やコードベースの要約が自動生成され、Ask Devin がこれを参照します。「Devin Review にインデックスが直接使われる」と明記したドキュメントはありませんが、Layer 3 で見た通り Devin Review は Ask Devin セッション経由で動いているので、間接的にインデックスとDeepWikiの恩恵を受けていると考えられます。
