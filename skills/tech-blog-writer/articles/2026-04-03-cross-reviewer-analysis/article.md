...
というのも、Devin Reviewはコードベース全体の文脈を理解した上でレビューしているからです。[コチラ](https://zenn.dev/dely_jp/articles/6fc3b9340be679)の記事でDevin Reviewの仕組みを解説したので、興味がある方はぜひ読んでいただきたいです。
...
## 15PRの定量分析 — 全員一致は1件だけだった

タイトルに「全員一致は0件」と書きましたが、15PRまで増やしたら1件だけありました。すいません。

フル4ツール並列実行（CR + PRT + CX + CXA）した8PRの指摘を全部突き合わせた結果です。
...
指摘の大半が1ツールだけの検出です。「ツール間で精度にばらつきがある」のではなく、「そもそも違う問題を見ている」のが実態でした。

唯一の全員一致は #7948 の `affiliates.sample` です。全アフィリエイトをメモリに読み込んで `.sample` しているコードを4ツール全員が指摘しました。ただ面白いのは、解釈が割れていること。Anthropic系は「全件メモリ読み込みのパフォーマンス問題」、Codex系は「`display_order` を無視している機能バグ」と判定しています。全員一致ですら、なぜ問題なのかで合意できていません。

### 重要度逆転は系統的に起きる

先ほどRAILS_ENVの例を1つ挙げましたが、15PRまで増やすと重要度逆転が系統的に発生していることが分かります。

| PR | 問題 | code-review | pr-review-toolkit | Codex系 | 差 |
|-----|------|------------|------------------|---------|---|
| #7948 | affiliates.sample | suggestion | critical | critical | 3段階 |
| #8042 | OpenAPI 400定義漏れ | info | critical | — | 3段階 |
| #7564 | RecordNotUnique未処理 | — | suggestion | critical | 3段階 |
| #7941 | pluck vs select | medium | critical | — | 2段階 |

同じ問題で3段階ずれるのは、スコアリングのキャリブレーション差ではなく、**評価軸そのものが違う**ことを示しています。code-reviewは「コードとして正しいか」、pr-review-toolkitは「プロジェクトの品質基準を満たしているか」、Codex系は「本番で何が壊れるか」を見ている。同じ問題でも、見ている角度が違えば重要度は変わります。

code-reviewの信頼度スコア80未満フィルタは偽陽性を減らす設計ですが、副作用として本当に重要な問題を「参考」レベルに落としてしまうケースがありました。#7936では高信頼度の指摘が0件で、3件すべてが参考レベル（スコア30〜55）に留まっています。pr-review-toolkitが同じPRでcritical 1件 + important 2件を出していることを考えると、精度と網羅性のトレードオフが見えます。

### adversarial-reviewはstandard reviewより探索が深い

8PR走らせた中で一貫しているのは、Codex adversarial-reviewのほうがCodex reviewより探索が深いことです。

| PR | Codex review | Codex adversarial | adversarialだけの発見 |
|----|-------------|-------------------|---------------------|
| #7948 | 1件 | 2件 | `created_at`フィルタの設計ミス |
| #8049 | 1件 | 3件 | ランタイム依存クラッシュ、トランザクション競合の誤分類 |
| #7564 | 1件 | 2件 | 招待コードのバリデーション漏れ |
| #7766 | 0件 | 2件 | レースコンディション |

adversarialのプロンプトに「デフォルトで懐疑的であれ」「コストの高い失敗を優先しろ」と書いてあるだけで、モデルがメソッドチェーンを深く追いかけるようになっています。#7948では、`created_at`でフィルタしているコードに対して「なぜ `display_start_at` / `display_end_at` カラムがあるのに使われていないのか？」と疑問を持ち、フィルタ設計自体の誤りを指摘しました。standard reviewはこの問いを立てていません。

プロンプトの姿勢指定が探索深度を変えるというのは、レビュースキル設計として重要な知見です。adversarial用の別プロンプトを用意するだけで、同じモデルから質の違うレビューが得られます。

### ツール間の問題空間は直交している

15PRの結果を俯瞰すると、ツールは「同じ問題を違う精度で見ている」のではなく、**そもそも違う問題空間を探索している**ことが分かります。

- **Anthropic系**: プロジェクトルール（CLAUDE.md / Rules）の違反検出。テストスタイル、OpenAPIスキーマ、YARD ドキュメントなど
- **Codex系**: diff外のコードとの不整合検出。定数の登録漏れ、フィルタ設計の誤り、既存実装との乖離
- **Devin**: ドメイン固有のインフラ差分検出。FCMトークン、テンプレートメソッドのオーバーライド漏れ、Railsのアンチパターン

検出対象がほぼ重ならないから、全員一致が起きない。これは欠点ではなく、複数ツールを走らせる理由そのものです。
