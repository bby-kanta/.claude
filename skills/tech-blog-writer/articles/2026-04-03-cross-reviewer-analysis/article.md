...
### code-review — CLAUDE.md に書いてあることしか指摘しない
...
### pr-review-toolkit — 内部で6つのサブエージェントが動く

1PRあたり5〜11件。code-review と比べると3〜4倍の指摘を出します。
...
ただ、code-review と比べるとノイズも多い。suggestion レベルの指摘が増えるぶん、「本当に直すべき問題」が埋もれがちです。
...
### Codex adversarial-review — 「このエンドポイント、悪用できない？」を考える

「このエンドポイントを悪意のあるリクエストで叩いたらどうなるか」を考えるレビュアーです。

リプレイ攻撃で同じリクエストを再送した場合の冪等性や、例外処理のrescueが処理を停止せずに後続の処理が走ってしまうケースなど、通常のレビューでは見落としがちな攻撃面を探します。

指摘も少ないですが、見つけた場合のインパクトはかなり大きい。既存コードに潜んでいたバグを掘り起こすこともあります。
