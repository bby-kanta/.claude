...
diff の中身を起点に、関連するモデル・サービス・クエリを `rg` と `sed` で次々に読みに行きます。実際の探索ログを見ると、あるPRでは差分のジョブクラスを起点に、コイン操作のモデル → 履歴表示のテンプレート → レポーティング用の集計クエリ → 類似ジョブの実装パターンと、5〜6ファイルを芋づる式にたどっていました。

この「diff の外を読む」挙動が、他のツールにはない検出力につながっています。「新しい値を追加したけど、その値を参照している別ファイルのホワイトリストに含まれていない」「レポーティングクエリの集計対象に入っていない」のような問題は、下流のファイルを実際に読まないと気づけません。

既存の同種実装（たとえば同じ親クラスを継承した別の子クラス）を探して新実装と比較する動きもします。「既存はこうなっているのに新しいほうだけ抜けている」という差分の検出に効いています。
