...
[プラグインのソースコード](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/code-review)を読むと、内部で **5つの Sonnet エージェントが並列に動いている**ことが分かります。
...
各エージェントの指摘に対して、Haiku が 0〜100 の信頼度スコアを付けます。CLAUDE.md 起因の指摘はダブルチェックが入り、**スコア80未満はすべてフィルタされます**。なので出力される指摘は少ないけど精度が高い、という挙動になります。

面白いのは Agent #2（バグスキャン）に対して "avoid reading extra context beyond the changes" と**意図的に diff の外を読まないよう指示している**ことです。code-review が diff 外の問題をスルーするのはモデルの限界ではなく、偽陽性を減らすための設計判断でした。
