# AIレビュー系のツール多すぎる。複数のAIレビュアーを同時に走らせたら、全員一致は0件だった。
...
### Codex review — diff から芋づる式にコードを読み進める

Codex review と adversarial-review は内部的にまったく別の仕組みで動いています。

Codex review は Codex の**ビルトインレビューア**を呼び出します。プラグインが `review/start` API を叩くと、Codex 側が diff を受け取って自律的にレビューを実行します。このとき Codex モデルは Read, Glob, Grep, Bash などのツールアクセスを持っていて、何を探索するかはモデルが自分で判断します。

実行ログに出てくる `rg` や `sed` での探索はプラグインがスクリプトで回しているのではなく、モデルが「この diff ならこのファイルも読んだほうがいいな」と判断して自律的に実行しています。あるPRでは差分のジョブクラスを起点に、コイン操作のモデル → 履歴表示のロジック → レポーティング用の集計クエリ → 類似ジョブの実装パターンと、5〜6ファイルを芋づる式にたどっていました。

この「モデルが自分で探索範囲を決める」挙動が、diff に含まれないファイルとの不整合を検出する力につながっています。既存の同種実装を探して新実装と比較する動きもするので、「既存はこうなっているのに新しいほうだけ抜けている」という差分の検出にも効きます。

指摘は1PRあたり2〜3件と少ないですが、重要度が高い傾向にあります。

### Codex adversarial-review — 「この設計、本当に大丈夫？」を問い続ける

adversarial-review は Codex のビルトインレビューアではなく、**カスタムプロンプト + 構造化JSON出力**で動きます。プラグイン内の [`prompts/adversarial-review.md` ](https://github.com/openai/codex-plugin-cc/blob/main/plugins/codex/prompts/adversarial-review.md)にプロンプトがあり、中身はこんな感じです:

「デフォルトで懐疑的であれ」「コストの高い失敗を優先しろ」という指示が明示的に入っています。
通常の review が「実装が正しいか」を確認するのに対して、adversarial-review は「この設計で想定外のケースに耐えられるか」を疑います。冪等性、例外処理の rescue が後続ロジックを止めないケース、下流のバリデーションとの不整合など、実装者が「うまくいくパス」しか想定していないときに刺さる指摘が多いです。

基本指摘は少ないですが、見つけた場合のインパクトが大きい。既存コードに潜んでいた設計上の穴を掘り起こすこともあります。
