...
5. **知りたいことがあれば`/llm-wiki query`** → Wiki内の知識を統合して回答が返ってくる
...
今回紹介した4つの設計判断（2層index、ソース整理、ページ判断基準、query保存基準）は自分のユースケースに合わせたものなので、別のドメインや別のワークフローなら違う判断になると思いますので、是非オリジナルのSKILL.mdを育成してみてください。
