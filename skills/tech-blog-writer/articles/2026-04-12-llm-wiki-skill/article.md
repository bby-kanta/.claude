...
**Wiki** — LLMが生成・管理するマークダウンファイルです。要約ページ、概念ページ、エンティティページ、比較ページなど。LLMが全て書いて、全て定期的にメンテナンスします。AIが理解しやすい文章に書き換えられている状態です、
...
**query（質問）** — Wikiに対して質問する。LLMがindexを読んで関連ページを探し、内容を読み込んで回答を合成する。参照先が生のドキュメントではなくすでに構造化されたWikiページなので、回答の質が良いです。更に良い回答はWikiに書き戻して残せます。
