...
RAGは、LLMに外部ドキュメントの知識を使わせる手法です。ドキュメントをベクトルDBに格納しておき、質問が来たら関連するチャンクを検索して、LLMのプロンプトに注入して回答を生成する。

5つのドキュメントを横断する質問をしたとき、LLMは関連する断片を見つけて、合成して、回答する。同じ質問を明日もう一度しても、まったく同じ検索と合成をゼロからやり直す。前回の回答は消えていて、知識としては残っていません。
...
LLM Wikiにはいくつか特有の概念があるので、先に整理しておきます。

| 用語 | 説明 |
|---|---|
| ソース（raw） | ユーザーが収集した元ドキュメント。本記事ではObsidian Web Clipperで保存したWeb記事や技術ドキュメントを指す。Wikiの原材料であり、LLMはこれを読むが中身は変更できない。ソースが不変であることで、Wikiにハルシネーションが混入するリスクを抑える |
| Wiki | LLMが生成・管理するマークダウンファイル群。ソースの要約、概念の解説、比較分析などがページとして整理され、ページ同士がリンクでつながっている |
| ジャンル | Ruby、TypeScriptなどトピック別のカテゴリ。Wikiのフォルダ分けに使う。ソースを取り込むときに内容から自動判定するか、ユーザーが指定する |
| ingest | ソースをWikiに取り込む操作。LLMがソースを読み、要約を作り、既存のWikiページに知識を統合し、ページ間の相互参照を張る。LLM Wikiのメイン操作 |
| query | Wikiに対して質問する操作。LLMがWiki内の関連ページを探して読み、内容を統合して回答を返す |
| lint | Wikiの健全性チェック。ページ間の矛盾、どこからもリンクされていない孤立ページ、まだ独立ページがない重要概念などを検出する |
| index | Wikiのカタログファイル。全ページの一覧とリンクが書かれていて、LLMがWikiを探索するときの起点になる |
| log | 操作の時系列記録。いつ何をingestしたか、どのページを作成・更新したかが記録される |
| Obsidian | マークダウンファイルをローカルで管理・閲覧するナレッジベースアプリ。ファイル間のリンクをグラフで可視化できるのが特徴 |
| Obsidian Web Clipper | Webページをマークダウン形式で保存するObsidian公式のブラウザ拡張。保存したファイルがそのままingestのソースになる |
| `[[wikilinks]]` | Obsidianのリンク記法。`[[ページ名]]`と書くと別のページへのリンクになる。Wikiページ間の相互参照に使う |

### 3層アーキテクチャ

LLM Wikiは3つのレイヤーで構成されます。

**Raw Sources（ソース）** — ユーザーが集めた生のドキュメントです。主にWebクリップで生成します。LLMはこれを読むが、絶対に更新しません。よってハルシネーションのリスクがない、信頼できる知識の源泉になります。

**Wiki** — LLMが生成・管理するマークダウンファイルです。要約ページ、概念ページ、エンティティページ、比較ページなど。LLMが全て書いて、全て定期的にメンテナンスします。

**Schema（スキーマ）** — Wikiの構造と規約を定義するドキュメントです。これにより会話に必要な知識に効率的にアクセスできます。
...
記述したものは既にブラッシュアップ済みですが、/skill-creatorが初期に生成したSKILL.mdは完璧ではありませんでした。
実際にingestを回してみると、概念文書を変換しただけでは足りない部分が出てきました。「これ新規ページにするの？既存ページに追記するの？」「indexが肥大化してきたけどどうする？」といった、実運用に必要な判断基準が足りていませんでした。
---

## 自分なりのアレンジ — 4つの設計判断

たろう眼鏡が追加した設計判断を4つ紹介します。

### 1. 2層index/log構造

最初のアウトプットではindexは1ファイル、logも1ファイルで。Wiki全体のページを1つのindexに全部載せる構成でした。
...
Obsidian Web ClipperでWebページを保存すると、`Clippings/`直下にフラットに溜まっていきます。最初は数件なので問題ないですが、20件、30件と増えるとソースが直列に沢山並んでしまい、どれがどのジャンルのソースなのか分からなくなります。

ingestの手順に「ソース原本をジャンル別フォルダに移動する」ステップを追加しました。

例えば、MySQLの記事をingestしたら、`Clippings/MySQL...md`が`Clippings/rdb/MySQL...md`に自動で移動されます。ソース要約ページの原本パスも更新されます。Wikiだけでなくソース側も最低限整理された状態が保たれます。
...
基準がないと、「全部保存」か「全部保存しない」のどちらかに偏ります。全部保存するとWikiが肥大化して質が下がるし、全部保存しないとqueryで得た知識が消えてもったいないです。
...
興味がある方は、[KarpathyのGist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)をそのままLLMに投げるだけで始められますよ！！
