...
OpenAI Codex モデルによる adversarial-review を実行し、その結果を返すこと。
...
```bash
sed -i '' '/^disable-model-invocation: true$/d' ~/.claude/plugins/marketplaces/openai-codex/plugins/codex/commands/adversarial-review.md
```
...
```bash
node ~/.claude/plugins/marketplaces/openai-codex/plugins/codex/scripts/codex-companion.mjs adversarial-review --wait --scope branch 2>&1
```
